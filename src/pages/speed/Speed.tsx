...
useEffect(() => {
  ...
  if (LeadsData?.data) {
    ...
    setTableData((prevData = []) => {
      ...
      console.log("offet", offset, "prevData length:", prevData.length, "New data length:", LeadsData.data.length);
      // Otherwise, merge new data with previous, avoiding duplicates
      const existingIds = new Set(prevData.map((i: any) => i?.id));
      console.log("prevData length:", prevData.length, "New data length:",existingIds );
      const merged = [
        ...prevData,
        ...LeadsData.data.filter((item: any) => !existingIds.has(item.id)),
      ];
      console.log("Merged data length:", merged.length, "Total leads:", LeadsData?.totalCount);
      return merged;
    });
...
console.log("offet", offset, filteredData.length, tableData.length)

// useEffect(() => {
//   // Only load more if there is more data to fetch
//   if (
//     filteredData.length < 40 &&
//     offset + 40 < totalLeads && // Only increment if not exceeding totalTasks
...
// }, [filteredData, offset, totalLeads]);
...
onClick={() => {
  ...
  withPagination={false}
   onIncrementNearEnd={() => {
      if (isLeadsDataFetching || isOffsetLoadingRef.current) return;
...
if (offset + 40 >= totalLeads) return;
 ...
 // Set ref immediately
      lastRequestedOffsetRef.current = offset + 40; // Track the new offset

      if(offset + 40 >= totalLeads) return; // Prevent incrementing beyond totalLeads
      
    setIsOffsetLoading(true);
    isOffsetLoadingRef.current = true;
      console.log("Incrementing offset to", offset, filteredData.length, tableData.length);
