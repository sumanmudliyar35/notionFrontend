...
const fetchLeadsByUser = async ({ queryKey }: QueryFunctionContext) => {
  ...
  const response = await axiosInstance.get(`/leads/getLeadsByUser?userid=${userId}&filters=${JSON.stringify(filters)}&limit=${limit}`);
  return response.data;
};

// Custom hook for fetching leads by user
export const useGetLeadsWithNoOffset = (userId: number | string, offset: number, filters: any, limit: number) => {
